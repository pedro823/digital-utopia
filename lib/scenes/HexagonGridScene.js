...
import { generateGrid, OFFSET_HEXAGONAL, OFFSET_SQUARE, SPACING_COLUMN_HEXAGONAL, SPACING_COLUMN_SQUARE, SPACING_ROW_HEXAGONAL, SPACING_ROW_SQUARE } from "../logic/generateGrid";
...
export default function HexagonGridScene() {
  ...
  return (
    <>
      {grid.map((center, idx) => (
        <Hexagon center={center} key={`hexagon-grid-${idx}`} radius={radius * 3} color="#00ff00" rotation={Math.PI / 2} />
      ))}
      {/* {grid.map((center, idx) => (

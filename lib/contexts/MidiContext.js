...
export const MidiProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchNavigatorAccess = async () => {
      try {
        ...
        console.error(
          'could not fetch access to MIDI controllers. Reload the page to request again. Underlying exception:',
          ex
        )
      }
    }
  ...
  return <MidiContext.Provider value={provision}>{children}</MidiContext.Provider>
}

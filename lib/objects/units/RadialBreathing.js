...
const circlePosition = (itemIndex, maxItems, layerOffsetTheta) => {
  ...
  return {
    theta: theta + layerOffsetTheta,
    position: [Math.cos(theta + layerOffsetTheta), Math.sin(theta + layerOffsetTheta)],
  }
}
...
export const RadialBreathing = forwardRef(function RadialBreathing(
  ...
  const createdChildren = useMemo(
    () =>
      new Array(layers).fill().flatMap((_, layerNumber) => {
        ...
        for (let i = 0; i < amountOfChildren; i++) {
          ...
          } = circlePosition(i, amountOfChildren, layerOffsetTheta)
  ...
  useFrame((_, delta) => {
    ...
    childrenRefs.current.forEach(child => {
      ...
      child.element.position.set(
        Math.cos(child.theta) * radius,
        Math.sin(child.theta) * radius,
        child.element.position.z
      )
    })
  })
